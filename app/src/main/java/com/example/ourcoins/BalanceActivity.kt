...
class BalanceActivity : AppCompatActivity(), NavigationView.OnNavigationItemSelectedListener {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        val toggle = ActionBarDrawerToggle(
            this, drawerbalance, toolbar, R.string.navigation_drawer_open, R.string.navigation_drawer_close
        )
        drawerbalance.addDrawerListener(toggle)
        toggle.syncState()

        navbalance.setNavigationItemSelectedListener(this)
    }
    override fun onBackPressed() {
        if (drawerbalance.isDrawerOpen(GravityCompat.START)) {
            drawerbalance.closeDrawer(GravityCompat.START)
        } else {
            super.onBackPressed()
        }
    }
